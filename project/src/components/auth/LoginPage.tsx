import React from 'react';
import { Monitor, Shield, Zap, Globe } from 'lucide-react';
import { SocialMediaLinks } from '../SocialMediaLinks';

interface LoginPageProps {
  onLogin: (credential: string) => void;
  isLoading: boolean;
}

export const LoginPage: React.FC<LoginPageProps> = ({ onLogin, isLoading }) => {
  const handleGoogleLogin = () => {
    // Simulate Google OAuth flow
    onLogin('mock_google_credential');
  };

  return (
    <div className="min-h-screen bg-primary flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        {/* Logo & Branding */}
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-4">
            <Monitor className="h-12 w-12 text-accent" />
            <div>
              <h1 className="text-3xl font-bold text-primary">RenderScope Pro</h1>
              <p className="text-accent font-medium">v2.0</p>
            </div>
          </div>
          <p className="text-secondary text-lg">
            Live Render Tracker & Project Manager
          </p>
        </div>

        {/* Features Preview */}
        <div className="bg-secondary/50 backdrop-blur-sm border border-primary rounded-xl p-6 mb-8">
          <h2 className="text-xl font-bold text-primary mb-4">What's New in v2.0</h2>
          <div className="space-y-3">
            <div className="flex items-center gap-3 text-primary">
              <Shield className="h-5 w-5 text-success" />
              <span>Google OAuth 2.0 Authentication</span>
            </div>
            <div className="flex items-center gap-3 text-primary">
              <Zap className="h-5 w-5 text-accent" />
              <span>System-wide Theme Engine</span>
            </div>
            <div className="flex items-center gap-3 text-primary">
              <Globe className="h-5 w-5 text-accent" />
              <span>Advanced Folder Management</span>
            </div>
          </div>
        </div>

        {/* Login Card */}
        <div className="bg-secondary border border-primary rounded-xl p-8">
          <h3 className="text-2xl font-bold text-primary text-center mb-6">
            Sign in to continue
          </h3>

          <button
            onClick={handleGoogleLogin}
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 bg-white text-black font-semibold py-3 px-4 rounded-lg hover:bg-gray-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <div className="h-5 w-5 border-2 border-gray-400 border-t-transparent rounded-full animate-spin" />
            ) : (
              <svg className="h-5 w-5" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
            )}
            {isLoading ? 'Signing in...' : 'Sign in with Google'}
          </button>

          <div className="mt-6 text-center text-sm text-secondary">
            By signing in, you agree to our Terms of Service and Privacy Policy
          </div>
        </div>

        {/* Social Media Links */}
        <div className="mt-8 flex justify-center">
          <SocialMediaLinks />
        </div>

        {/* Footer */}
        <div className="text-center mt-8 text-secondary text-sm">
          <p>© 2025 RenderScope Pro. All rights reserved.</p>
          <p className="mt-1">Powered by ArchViz Trader</p>
          <p className="mt-1">Production-Ready Dashboard for Animation & VFX</p>
        </div>
      </div>
    </div>
  );
};